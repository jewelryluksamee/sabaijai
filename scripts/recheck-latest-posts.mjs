// Run: node --env-file=.env.local scripts/recheck-latest-posts.mjs
// Re-detects emotion for the N latest posts regardless of current emotion value
...
async function main() {
  ...
  console.log(`กำลังตรวจสอบ ${snapshot.size} โพสล่าสุด\n`);

  for (const doc of snapshot.docs) {
    const { content, emotion: oldEmotion } = doc.data();
    if (!content) { console.log(`- ${doc.id}: ไม่มี content ข้าม`); continue; }
    ...
    try {
      ...
      const changed = oldEmotion !== result.category ? ` (เดิม: ${oldEmotion ?? "ไม่มี"})` : "";
      console.log(`✓ ${doc.id}: ${result.category}${changed}`);
